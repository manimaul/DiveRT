    ...
    def fmt_lon(self, fmt='DMM'):
        ...
        if self.data_long is None:
            ...
            if fmt == 'DMM': #.0001 minute is roughly .6 feet @ equator
                ...
                if deg < 0.0:
                    prlon = str(-(deg)) + '*' + str(-(min)) + '\'W'
                if deg == 0.0:
                    prlon = str(deg) + '*' + str(min) + '\''
                if deg > 0.0:
                    prlon = str(deg) + '*' + str(min) + '\'E'
                return prlon.encode('utf-8')
            if fmt == 'DMS': #.001 second is roughly 1 foot @ equator
                ...
                if deg < 0.0:
                    prlon = str(-(deg)) + '*' + str(-(min)) + '\'' + str(-(sec)) + '\"W'
                if deg == 0.0:
                    prlon = str(deg) + '*' + str(min) + '\'' + str(sec) + '\"'
                if deg > 0.0:
                    prlon = str(deg) + '*' + str(min) + '\'' + str(sec) + '\"E'
                return prlon.encode('utf-8')
    ...
    def fmt_lat(self, fmt='DMM'):
        ...
        if self.data_lat is None:
            ...
            if fmt == 'DMM': #.0001 minute is roughly .6 feet @ equator
                ...
                min = round((min * 60), 4) #this does not actually round up or down
                if deg < 0.0:
                    prlat = str(-(deg)) + '*' + str(-(min)) + '\'S'
                if deg == 0.0:
                    prlat = str(deg) + '*' + str(min) + '\''
                if deg > 0.0:
                    prlat = str(deg) + '*' + str(min) + '\'N'
                return prlat.encode('utf-8')
            if fmt == 'DMS': #.001 second is roughly 1 foot @ equator
                deg = int(lat)
                mnn = (lat - deg) * 60
                ...
                sec = round(((mnn - min) * 60), 3)
                if deg < 0.0:
                    prlat = str(-(deg)) + '*' + str(-(min)) + '\'' + str(-(sec)) + '\"S'
                if deg == 0.0:
                    prlat = str(deg) + '*' + str(min) + '\'' + str(sec) + '\"'
                if deg > 0.0:
                    prlat = str(deg) + '*' + str(min) + '\'' + str(sec) + '\"N'
                return prlat.encode('utf-8')
...
if __name__== "__main__":
    ...
    import os
    import threading
    threading.Timer(10, GPS.close).start() #schedule close in 30 seconds
